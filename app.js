...
const integrityFragmentSource = "社会达尔文主义|西方进化论|社会进化论|中国近代思想|近代中国|俱分进化论|建立宗教论|国故论衡|阿赖耶识|真如本体|不住生死|不住涅槃|清末民初|近代科学|转俗成真|现代性|现代化|出世性|唯识学|今文经学|齐物论|黑格尔|海格尔|达尔文|斯宾塞尔|斯宾塞|阪元弘子|同盟会|民报|章太炎|康有为|欧阳竟无|王阳明|熊十力|梁启超|严复|周展安|张志强|太炎|佛教|佛学|儒学|庄学|易学|理学|西学|八识|三籁|乾坤|真如|良知|哲学|政治|历史|文献";

function knownIntegrityFragments(text) {
  return uniqueItems(Array.from(String(text || "").matchAll(new RegExp(integrityFragmentSource, "g"))).map((match) => match[0]));
}

function normalizeLockCandidates(term) {
  const clean = String(term || "")
    .replace(/^[的其是以把将对当而则与和及于在向从为由所]+/, "")
    .replace(/[，。；：、,.!?！？；:]+$/g, "")
    .trim();
  if (!clean) return [];
  const fragments = knownIntegrityFragments(clean);
  if (fragments.length && !fragments.includes(clean) && (clean.length > 6 || /^(他|其|以|把|将|会通|通过|当|由|从|对)/.test(clean))) return fragments;
  if (clean.length > 10 && fragments.length) return fragments;
  return [clean];
}

function normalizedTermText(text) {
  return String(text || "").replace(/[《》“”"'‘’\s，。、；：:;,.!?！？（）()[\]【】<>]/g, "");
}

const integrityAliases = {
  章太炎: ["章太炎", "章氏", "太炎"],
  太炎: ["章太炎", "章氏", "太炎"],
  章氏: ["章太炎", "章氏", "太炎"],
  黑格尔: ["黑格尔", "海格尔"],
...
function isHardIntegrityTerm(term) {
  const clean = normalizedTermText(term);
  if (!clean || /^(思想|关系|问题|价值|主体|传统|现代|佛教|学术|文本|文献|解释|结构|论证)$/.test(clean)) return false;
  if (/文集|页码|出版社|版本|注释/.test(clean) && clean.length > 6) return false;
  const fragments = knownIntegrityFragments(clean);
  if (fragments.includes(clean)) return true;
  if (fragments.length >= 2 && clean.length <= 12) return true;
  if (/[A-Za-z0-9]{2,}|[12][0-9]{3}/.test(clean)) return true;
  return clean.length <= 8 && /(论|学|识|史|经|佛|儒|庄|法|宗|义|理)$/.test(clean);
}

function hasIntegrityTerm(text, term) {
  const output = normalizedTermText(text);
  const cleanTerm = normalizedTermText(term);
  if (!cleanTerm) return true;
  if (output.includes(cleanTerm)) return true;
  const aliases = integrityAliases[cleanTerm] || [];
  if (aliases.some((alias) => output.includes(normalizedTermText(alias)))) return true;
  const fragments = knownIntegrityFragments(cleanTerm).filter((fragment) => normalizedTermText(fragment).length >= 2);
  if (fragments.length >= 2) {
    const hits = fragments.filter((fragment) => output.includes(normalizedTermText(fragment))).length;
    return hits >= Math.min(2, fragments.length);
  }
  return false;
}

function isRiskyAdditionTerm(term) {
  const clean = normalizedTermText(term);
  if (!clean || clean.length < 2) return false;
  if (/^(问题|关系|解释|文本|文献|思想|学术|传统|现代|主体|价值|结构|论证|意义|必然|时期|此一思想)$/.test(clean)) return false;
  if (/[12][0-9]{3}|[A-Za-z]{2,}/.test(clean)) return true;
  const fragments = knownIntegrityFragments(clean);
  return fragments.some((fragment) => !/^(佛教|佛学|儒学|庄学|哲学|政治|历史|文献)$/.test(fragment));
}

function paragraphCount(text) {
  return splitParagraphs(text).length || (String(text || "").trim() ? 1 : 0);
}

function paragraphRule(lock) {
  const count = lock.paragraphs || 1;
  if (count <= 1) return "原文是 1 段，最终改写正文必须严格保持 1 段，不得拆成多段，不得另起标题。";
  return `原文是 ${count} 段，最终改写正文必须保持 ${count} 段，段落顺序与原文一致。`;
}

function sourceLockTerms(text, profile = null, limit = 36) {
  const quoted = Array.from(text.matchAll(/[《“"]([^》”"]{2,24})[》”"]/g)).map((match) => match[1]);
  const explicit = knownIntegrityFragments(text);
  ...
  return uniqueItems([...explicit, ...quoted, ...synthetic, ...conceptual, ...profileHits].flatMap(normalizeLockCandidates))
...
function buildSemanticLock(text, profile, mode = "article", longformMode = "1200") {
  ...
  return {
    ...
    paragraphs: paragraphCount(clean),
...
- 输入规模: ${lock.chars} 字符；${lock.sentences} 个句子；${lock.paragraphs} 个段落；${lock.chunks.length} 个材料块。
- 段落约束: ${paragraphRule(lock)}
...
function extractModelDraftBody(text) {
  let clean = normalizeIdeaText(text);
  if (!clean) return "";
  clean = clean.replace(/^#{1,6}\s*风格化改写稿\s*/u, "").trim();
  const stopPatterns = [
    /\n#{1,6}\s*简短核验/u,
    /\n#{1,6}\s*本地保真复核/u,
    /\n#{1,6}\s*Codex\s*风格自检/u,
    /\n简短核验[:：]/u,
    /\n内容覆盖[:：]/u,
  ];
  const cut = stopPatterns
    .map((pattern) => {
    ...
    .sort((a, b) => a - b)[0];
  if (typeof cut === "number") clean = clean.slice(0, cut).trim();
  return clean.replace(/^#{1,6}\s*风格化改写稿\s*/u, "").trim();
}

function contentIntegrityAudit(sourceText, outputText, profile = null) {
  const source = normalizeIdeaText(sourceText);
  const output = normalizeIdeaText(extractModelDraftBody(outputText));
  const lock = buildSemanticLock(source, profile || activeProfile());
  const requiredTerms = lock.requiredTerms.filter((term) => term.length >= 2 && isHardIntegrityTerm(term)).slice(0, 24);
  const softTerms = lock.requiredTerms.filter((term) => term.length >= 2 && !requiredTerms.includes(term)).slice(0, 18);
  const hasTerm = hasIntegrityTerm;
  ...
  const riskyAdditions = outputTerms
    .filter((term) => !sourceTerms.has(term) && !styleTerms.has(term) && !hasTerm(source, term))
    .filter(isRiskyAdditionTerm)
    .filter((term) => !/^(问题|关系|解释|文本|文献|思想|学术|传统|现代|主体|价值|结构|论证)$/.test(term))
    .slice(0, 12);
  const chunkCoverage = lock.chunks.map((chunk) => {
    const terms = chunk.terms.filter((term) => term.length >= 2 && isHardIntegrityTerm(term)).slice(0, 8);
    const missing = terms.filter((term) => !hasTerm(output, term));
    const coverage = terms.length ? (terms.length - missing.length) / terms.length : 1;
    return {
      id: chunk.id,
      ok: coverage >= 0.6,
      coverage,
      missing: coverage >= 0.6 ? [] : missing,
    };
  });
  ...
  const chunkScore = chunkCoverage.length
    ? Math.round((chunkCoverage.reduce((sum, chunk) => sum + chunk.coverage, 0) / chunkCoverage.length) * 100)
    : 100;
  const hallucinationPenalty = riskyAdditions.length ? Math.min(25, riskyAdditions.length * 4) : 0;
  const score = Math.max(0, Math.min(100, Math.round((termScore * 0.62) + (chunkScore * 0.38) - hallucinationPenalty)));
  return {
    score,
...
function formatIntegrityAuditMarkdown(audit) {
  ...
  const chunkWarnings = audit.chunkCoverage
    ...
    .map((chunk) => `- ${chunk.id} 覆盖偏低：${Math.round(chunk.coverage * 100)}%；缺少：${chunk.missing.join("、")}`)
    .join("\n") || "- 分块关键词覆盖通过。";
  return `## 本地保真复核
...
6. 最终只输出改写正文；不要输出标题、简短核验、项目符号或后台分析过程。
...
16. 段落数量必须服从语义锁定包的“段落约束”：原文一段就只输出一段；原文多段就按同样段数、同样顺序迁移，不得为了显得完整而拆段扩写。
...
最终输出约束：
- 只输出改写后的正文，不要标题，不要“风格化改写稿”字样，不要“简短核验”。
- ${paragraphRule(lock)}
- 每一段只对应原文同序段落，不要把一段原文扩成多段。
- 开头先建立历史/思想/概念问题域，不要直接宣布结论。
- 开头句式必须根据原文和 profile 变化，不要机械使用“要理解……不能只……”。
- 必须逐块覆盖语义锁定包里的 [C1] [C2] [C3]...，不得只改第一句。
- 语义锁定包里的“必保关键词/专名”必须尽量保留；如果为了文体需要替换，只能在正文中用同义结构承接，不要另写说明。
...
正文要求：
- 用中文学术文体写作。
- 只输出正文，不要标题、不要简短核验、不要项目符号。
- 严格遵守段落数量约束：原文一段只输出一段；原文多段按同段数同顺序输出。
...
async function generateStrictModelDraft(profile, idea, mode, longformMode = "1200") {
  ...
  const rawDraft = await callOpenAIResponse(prompt);
  const draft = extractModelDraftBody(rawDraft) || rawDraft;
